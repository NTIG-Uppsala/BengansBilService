...
function isDateClosed(date) {
  ...
  const closedDays = [
    // Array of all closed days months are calculated from 0-11
...
function setOpeningStatus(date) {
  ...
  if (hour === openingTime - 1 && minute >= 30) {
    // Closed but max 30 min before opening
    displayOpeningStatus(`Öppnar om ${60 - minute} minuter`, "orange");
  } else if (hour < openingTime) {
    // Closed and more than 30 min until open today
    displayOpeningStatus(`Öppnar idag kl. ${openingTime}`, "orange");
  } else if (hour === closingTime - 1 && minute >= 45) {
    // Open and 15 min or less before closing
    displayOpeningStatus("Stänger snart", "orange");
  } else if (hour >= openingTime && hour < closingTime) {
    // Open and more than 15 min before closing
...
function displayOpeningStatus(status, color) {
  ...
  if (color === "orange") {
    ...
    // If the store is opened and more than 15 minutes before closing
